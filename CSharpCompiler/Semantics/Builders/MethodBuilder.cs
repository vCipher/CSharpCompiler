...
{
    public sealed class MethodBuilder : IMethodDefinitionResolver, 
        IArgumentVisitor, IVarDeclarationVisitor, 
        ISyntaxTreeVisitor, IExpressionVisitor, IStatementVisitor
    {
        ...
        public static MethodDefinition Build(SyntaxTree syntaxTree, TypeDefinition typeDef, SemanticContext context)
        {
            var builder = new MethodBuilder(typeDef, context);
        ...
        {
            var typeInfo = TypeInference.InferType(node.ContainedType, _context);
        ...
        private bool NeedStackBalancing(InvokeExpression node, IMethodInfo method)
        {
            ...
            if (method.ReturnType.Equals(KnownType.Void)) return false;
